...
DevPilot 当前是一个基于 Vue 3、Vite、TypeScript 和 Tauri 2 的桌面应用骨架。项目目标是先建立稳定、可运行、可扩展的桌面应用基础，再逐步补充具体产品功能。

## 2. 产品定位

DevPilot 定位为本地优先的桌面工具应用。初期重点不是完整业务闭环，而是搭建可维护、可持续扩展的应用底座。

## 3. 目标用户

- 项目开发者：需要一个清晰的 Vue + Tauri 桌面应用起点。
- 后续终端用户：需要稳定、响应快、可离线运行的桌面工具。
